...
class DigitalMarketplace(ARC4Contract):
    ...
    # buy the asset
    @arc4.abimethod
    def buy(self, buyerTnx: gtxn.PaymentTransaction, quantity: UInt64) -> None:
        assert self.unitaryPrice != UInt64(0)

        assert Txn.sender == buyerTnx.sender
        assert buyerTnx.receiver == Global.current_application_address
        assert buyerTnx.amount == self.unitaryPrice * quantity
